...
flowchart LR
  subgraph City_of_New_Haven[City / Public Works]
    C1[Forecast demand & set minimum stock trigger]
    C2[Issue PO / release under contract]
  ...
  subgraph Supplier[Supplier / Producer]
    S1[Confirm allocation & quality spec]
    S2[Stage bulk salt at source/terminal]
    S3[Release shipment docs]
  end

  subgraph Terminal[Terminal Operator]
    T1[Receive vessel/barge/rail inbound]
    T2[Store/transload bulk inventory]
  ...
  subgraph Transporter[Transporter]
    ...
    R3[Queue management during storm peaks]
  end

  subgraph Regulator[Regulator (EPA/CT DEEP permit framework)]
    G1[Set stormwater permit obligations + BMP expectations]
    G2[Require SOP/inspection/corrective-action evidence]
    G3[Enforcement if discharge/permit failures occur]
  end
  ...
  C4 -. operational impacts/incident logs .-> G2
  G1 --> G2 --> G3
```
